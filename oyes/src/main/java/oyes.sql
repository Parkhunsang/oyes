...
-- 추가
  ...
  -- doctor_phd 컬럼 삭제
  ALTER TABLE doctor DROP COLUMN doctor_phd;

  -- doctor_graduation 컬럼 삭제
  ALTER TABLE doctor DROP COLUMN doctor_graduation;
...
insert into clinic_session values((select nvl(max(clinic_id)+1,1) from clinic_session), 7,'외래진료',to_date('2025-09-11 12:01','yyyy-mm-dd HH:mi'));
...
-- doctor_extra Data(doctor_id가 1인 데이터)
...
create table compliment (
    compliment_no   number(10)      PRIMARY KEY,
    patient_no      number(10)      not null,
    compliment_open number(1)       not null,
    compliment_title varchar2(60)    not null,
    compliment_for  varchar2(30),
    doctor_id       number(19),
    compliment_content varchar2(1000)  not null,
    create_date     date            not null,
    is_active       char(1)         default 'y' not null,
